    ...
    public partial class Groups : UserControl
    {
        ...
        private void btn_location_search_Click(object sender, EventArgs e)
        {
            using (SqlConnection sqlcon = new SqlConnection(connectionString))
            {
                ...
                SqlCommand cmd = sqlcon.CreateCommand();
                cmd.CommandType = CommandType.Text;
                cmd.CommandText = "SELECT  * FROM ( SELECT Date,Start_Time ,concat(Lecture_name,',',Subject_Name,',',Subject_Code,',',Room_Name) as con FROM tbl_session_data pp where pp.Group_No = '" + txt_timetable_group.Text + "') d PIVOT ( max(con) FOR Date IN ([Monday], [Tuesday],[Wednesday],[Thursday],[Friday],[Saturday],[Sunday]) ) as p ";
                cmd.ExecuteNonQuery();
                DataTable dt = new DataTable();
                SqlDataAdapter da = new SqlDataAdapter(cmd);
                da.Fill(dt);
